...
expName = 'Exercise_for_hcy'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/sunlinghan/PycharmProjects/Psychopy/Exercises/Exercise_for_hcy/Exercise_for_hcy_lastrun.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        ...
        win = visual.Window(
            size=[720, 600], fullscr=False, screen=0,
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    text_Instruction = visual.TextStim(win=win, name='text_Instruction',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_space = keyboard.Keyboard()
    ...
    text_Answer01 = visual.TextStim(win=win, name='text_Answer01',
        text='焦虑的        抑郁的',
        font='Arial Unicode MS',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    key_resp_answer01 = keyboard.Keyboard()
    ...
    text_Q2 = visual.TextStim(win=win, name='text_Q2',
    ...
    text_answer02 = visual.TextStim(win=win, name='text_answer02',
        text='焦虑的       抑郁的',
        font='Arial Unicode MS',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    key_resp_answer02 = keyboard.Keyboard()
    ...
    WelcomeScreenComponents = [text_welcome]
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        if text_welcome.status == STARTED:
            ...
            if tThisFlipGlobal > text_welcome.tStartRefresh + 2.0-frameTolerance:
    ...
    # keep track of which components have finished
    InstructionComponents = [text_Instruction, key_resp_space]
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        if text_blank.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_blank.tStartRefresh + .5-frameTolerance:
                # keep track of stop time/frame for later
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        # if text_Q1 is stopping this frame...
        if text_Q1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_Q1.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                text_Q1.tStop = t  # not accounting for scr refresh
                text_Q1.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'text_Q1.stopped')
                # update status
                text_Q1.status = FINISHED
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Answer01.started', globalClock.getTime())
    key_resp_answer01.keys = []
    key_resp_answer01.rt = []
    _key_resp_answer01_allKeys = []
    # keep track of which components have finished
    Answer01Components = [text_Answer01, key_resp_answer01]
    for thisComponent in Answer01Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # if text_Answer01 is starting this frame...
        if text_Answer01.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            text_Answer01.frameNStart = frameN  # exact frame index
            text_Answer01.tStart = t  # local t and not account for scr refresh
            text_Answer01.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_Answer01, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_Answer01.started')
            # update status
            text_Answer01.status = STARTED
            text_Answer01.setAutoDraw(True)
        
        # if text_Answer01 is active this frame...
        if text_Answer01.status == STARTED:
            # update params
            pass
        
        # if text_Answer01 is stopping this frame...
        if text_Answer01.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_Answer01.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
                text_Answer01.tStop = t  # not accounting for scr refresh
                text_Answer01.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'text_Answer01.stopped')
                # update status
                text_Answer01.status = FINISHED
                text_Answer01.setAutoDraw(False)
        
        # *key_resp_answer01* updates
        waitOnFlip = False
        
        # if key_resp_answer01 is starting this frame...
        if key_resp_answer01.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            key_resp_answer01.frameNStart = frameN  # exact frame index
            key_resp_answer01.tStart = t  # local t and not account for scr refresh
            key_resp_answer01.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(key_resp_answer01, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'key_resp_answer01.started')
            # update status
            key_resp_answer01.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(key_resp_answer01.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(key_resp_answer01.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if key_resp_answer01.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_answer01.getKeys(keyList=['a','l',], ignoreKeys=["escape"], waitRelease=False)
            _key_resp_answer01_allKeys.extend(theseKeys)
            if len(_key_resp_answer01_allKeys):
                key_resp_answer01.keys = _key_resp_answer01_allKeys[-1].name  # just the last key pressed
                key_resp_answer01.rt = _key_resp_answer01_allKeys[-1].rt
                key_resp_answer01.duration = _key_resp_answer01_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in Answer01Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Answer01.stopped', globalClock.getTime())
    # check responses
    if key_resp_answer01.keys in ['', [], None]:  # No response was made
        key_resp_answer01.keys = None
    thisExp.addData('key_resp_answer01.keys',key_resp_answer01.keys)
    if key_resp_answer01.keys != None:  # we had a response
        thisExp.addData('key_resp_answer01.rt', key_resp_answer01.rt)
        thisExp.addData('key_resp_answer01.duration', key_resp_answer01.duration)
    thisExp.nextEntry()
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        # if text_Q2 is starting this frame...
        if text_Q2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            text_Q2.frameNStart = frameN  # exact frame index
            text_Q2.tStart = t  # local t and not account for scr refresh
            text_Q2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_Q2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_Q2.started')
            # update status
            text_Q2.status = STARTED
            text_Q2.setAutoDraw(True)
        
        # if text_Q2 is active this frame...
        if text_Q2.status == STARTED:
            # update params
            pass
        
        # if text_Q2 is stopping this frame...
        if text_Q2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_Q2.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                text_Q2.tStop = t  # not accounting for scr refresh
                text_Q2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'text_Q2.stopped')
                # update status
                text_Q2.status = FINISHED
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Answer02.started', globalClock.getTime())
    key_resp_answer02.keys = []
    key_resp_answer02.rt = []
    _key_resp_answer02_allKeys = []
    # keep track of which components have finished
    Answer02Components = [text_answer02, key_resp_answer02]
    for thisComponent in Answer02Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # if text_answer02 is starting this frame...
        if text_answer02.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            text_answer02.frameNStart = frameN  # exact frame index
            text_answer02.tStart = t  # local t and not account for scr refresh
            text_answer02.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_answer02, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_answer02.started')
            # update status
            text_answer02.status = STARTED
            text_answer02.setAutoDraw(True)
        
        # if text_answer02 is active this frame...
        if text_answer02.status == STARTED:
            # update params
            pass
        
        # if text_answer02 is stopping this frame...
        if text_answer02.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_answer02.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                text_answer02.tStop = t  # not accounting for scr refresh
                text_answer02.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'text_answer02.stopped')
                # update status
                text_answer02.status = FINISHED
                text_answer02.setAutoDraw(False)
        
        # *key_resp_answer02* updates
        waitOnFlip = False
        
        # if key_resp_answer02 is starting this frame...
        if key_resp_answer02.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            key_resp_answer02.frameNStart = frameN  # exact frame index
            key_resp_answer02.tStart = t  # local t and not account for scr refresh
            key_resp_answer02.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(key_resp_answer02, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'key_resp_answer02.started')
            # update status
            key_resp_answer02.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(key_resp_answer02.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(key_resp_answer02.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if key_resp_answer02.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_answer02.getKeys(keyList=['a','l',], ignoreKeys=["escape"], waitRelease=False)
            _key_resp_answer02_allKeys.extend(theseKeys)
            if len(_key_resp_answer02_allKeys):
                key_resp_answer02.keys = _key_resp_answer02_allKeys[-1].name  # just the last key pressed
                key_resp_answer02.rt = _key_resp_answer02_allKeys[-1].rt
                key_resp_answer02.duration = _key_resp_answer02_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in Answer02Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Answer02.stopped', globalClock.getTime())
    # check responses
    if key_resp_answer02.keys in ['', [], None]:  # No response was made
        key_resp_answer02.keys = None
    thisExp.addData('key_resp_answer02.keys',key_resp_answer02.keys)
    if key_resp_answer02.keys != None:  # we had a response
        thisExp.addData('key_resp_answer02.rt', key_resp_answer02.rt)
        thisExp.addData('key_resp_answer02.duration', key_resp_answer02.duration)
    thisExp.nextEntry()
